...
Lived in ``planner.pricing`` until card 08; moved here because the Developer agent needs
the same table and agents must not depend on each other. Phase 0 has no model router
(that lands in Phase 2), so the table is deliberately hardcoded. Numbers are per 1M
tokens; unknown models return ``None`` so the row still persists but doesn't claim a
bogus cost.
"""

from decimal import Decimal

# (input_per_million, output_per_million) — Claude public pricing tiers.
